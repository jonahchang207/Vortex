...
void Chassis::swingToHeading(float theta, DriveSide side, int timeout,
                              SwingToHeadingParams params, bool async) {
    if (async) {
        pros::Task([this, theta, side, timeout, params]() {
            swingToHeading(theta, side, timeout, params, false);
        });
        pros::delay(10);
        return;
    }
    ...
    double startTheta = getPose().theta;
    distTraveled      = 0;
    double prevPower  = 0;
    bool   settling   = false;
    ...
    while (!timer.isDone() &&
           ...
           this->motionRunning)
    {
        double currentTheta = getPose().theta;
        ...
        // Only one side moves; the other is braked at 0.
